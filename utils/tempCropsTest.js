const cropData = require("./tempTestCropData.json");
...
const CRITICAL_THRESHOLDS = {
  temperature: {
    min: 5, // Most crops can't survive below 5°C
    max: 40, // Most crops can't survive above 40°C
  },
  ph: {
    min: 4.0, // Too acidic
    max: 9.0, // Too alkaline
  },
  humidity: {
    min: 30, // Too dry
    max: 95, // Too humid
...
function getBestCropMatch(sensorData) {
  ...
  // If no viable crops found
  if (!bestMatch) {
    return {
      success: false,
      message: "No suitable crops found due to critical condition failures",
...
const match = getBestCropMatch(testData);
